...
(function (domGlobals) {
  ...
  var hasListTab = function (editor) {
    return editor.getParam('image_list', true, 'boolean');
  };
  ...
  var collect = function (editor) {
    ...
    var hasListTab = Settings.hasListTab(editor);
  ...
  var makeTab$3 = function (info) {
    var items = [{
      type: 'htmlpanel',
      html: createHtml(info.imageList)
    }];
    return {
      title: 'List',
      name: 'list',
    ...
    if (info.hasAdvTab || info.hasUploadUrl || info.hasUploadHandler) {
      ...
      var tabPanel = {
        ...
        tabs: flatten([
          ...
          info.hasListTab ? [ListTab.makeTab(info)] : []
        ])
      };
      return tabPanel;
    } else {
  ...
  var createHtml = function (_imageList) {
    console.log(_imageList)
    var imageList = flatten( _imageList.toArray() )
    var contentDiv = document.createElement('div'); 

    contentDiv.setAttribute('class', 'mce-eco-panel-content');
    for (var imageIndex = 1; imageIndex < imageList.length; imageIndex++) {
      console.log(imageList[imageIndex])
      var title = imageList[imageIndex].text || imageList[imageIndex].title;
  ...
  function Plugin() {
    global$1.add('image', function (editor, url) {
      FilterContent.setup(editor, url);
